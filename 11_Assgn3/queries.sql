...
FROM
    ...
    main_class AS main
WHERE
    main.ID = Nomination_MainClass.ID
    AND Nomination_MainClass.Won = 1
GROUP BY
    Nomination_MainClass.ID,
    main.Original_title
HAVING
...
WITH experience (
    ...
    FROM
        part_of_movie AS PM
    WHERE
        PM.JOB IN ('director', 'assistant director')
...
SELECT
    experience.SSN,
    0.3 * experience.expcnt + 0.7 * (0.8 * (COALESCE(ad.avg_dir_rate, 0)) + 0.2 * (COALESCE(asd.avg_asstdir_rate, 0))) AS Score
FROM
    experience,
    avg_dir AS ad
    FULL OUTER JOIN avg_asstdir AS asd ON ad.SSN = asd.SSN
WHERE
    experience.SSN = ad.SSN
...
FROM
    ...
    (
        SELECT
            *,
            row_number() OVER (PARTITION BY E.Release_date ORDER BY E.run_time ASC) AS runtimerank
        FROM
            episode_releases_in AS E) AS releasesIn
WHERE
    releasesIn.run_time IS NOT NULL
    AND ep.episodeid = releasesIn.episodeid
    AND releasesIn.runtimerank = 1
ORDER BY
